...
export class EventsGateway implements OnGatewayInit{
  ...
  handleMessage(client: Socket, state: any) {
    ...
    this.wss.sockets.sockets.get(`${client.id}`).to(state.__source).emit('chatToClient',state)
  ...
  handleJoinRoom(client:Socket, state: any){
    ...
    this.depNews.map(v => {
      if (v.__hasread.indexOf(state.__sender) === -1 && Date.parse((new Date() as any)) - v.__date <= (1000 * 60 * 50 * 24 * 100)) {
        ...
        console.log(this.wss.sockets.sockets.get(`${client.id}`), 'svviasupoasubyvop');
        // setTimeout(() => {
          if (!!this.wss.sockets.sockets.get(`${client.id}`)) {
            console.log('新进房间');
            this.wss.sockets.sockets.get(`${client.id}`).to(v.__source).emit('chatToClient',{__sender:v.__sender,__message:v.__message,__source:v.__source})
          }

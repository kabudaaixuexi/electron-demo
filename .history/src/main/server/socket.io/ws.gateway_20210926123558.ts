  ...
  handleJoinRoom(client:Socket, state: any){
    ...
    this.depNews.map(v => {
      if (v.__hasread.indexOf(state.__sender) === -1 && Date.parse((new Date() as any)) - v.__date <= (1000 * 60 * 50 * 24 * 100)) {
        ...
        setTimeout(() => {
          this.wss.sockets.sockets[`${client.id}`] && this.wss.sockets.sockets[`${client.id}`].to(v.__source).emit('chatToClient',{__sender:v.__sender,__message:v.__message,__source:v.__source})

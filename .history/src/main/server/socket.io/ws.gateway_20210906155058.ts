...
export class EventsGateway {
  ...
  onIdentity(client: any, __From: any): Observable<WsResponse<any>> | any{
    ...
    __From.name === 'success' && client.emit('events', `通道identity建立通讯，服务地址为${client.handshake.headers.host},${new Date(client.handshake.time).getTime()}`)
    switch (__From.name){
      case 'hey':
        // let count = 0
  ...
  onEvent(client: any, __From: any): Observable<WsResponse<any>> | any {
    __From.name === 'success' && client.emit('events','events开始推送')

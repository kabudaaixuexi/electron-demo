    ...
    this.depNews.map(v => {
      if (v.__hasread.indexOf(state.__sender) === -1 && Date.parse((new Date() as any)) - v.__date <= (1000 * 60 * 50 * 24 * 100)) {
        ...
        setTimeout(() => {
          ...
          console.log(this.wss.sockets.sockets[`${client.id}`], 'idididididi');
          // this.wss.sockets.sockets[`${client.id}`].emit('chatToClient',{__sender:v.__sender,__message:v.__message,__source:v.__source})

...
import { fileBySuffix, appendFile, readFile, access } from '../__modular/file';
import { nestAssetsUrl } from '../../config/StaticPath'
import { createWriteStream } from 'fs';

@Injectable()
export class UploadService {
    async upload(files, body) : Promise<any> {
        console.log(files,'files');
        console.log(body,'body');
        console.log(await access(`${process.env.__static}/resources/desk`))
        for (const file of files) {
            const writeImage = createWriteStream(`${process.env.__static}/resources/desk/${body.name}-${Date.now()}-${file.originalname}`);

...
export default defineComponent({
  ...
  setup() {
    ...
    const sendNew = () => {
      socket.emit('chatToServer', { __sender: storeStateChat['userinfo'].uid, __message: state.newInput, __source: currentRoom.value.__source });
      state.newInput = ''
    }
    ...
    onUnmounted(() => {
      // 全部离开
      state.chatroomList.map(v => {
        ElMessage({ type: "error", message: `${storeStateChat['userinfo'].uid}退出房间${v.__source}` });

let ZINDEX = Number(1)
// 可拖拽组件：pc和Mobile
function useMove(el?:any, els?: any[], reset: boolean = false) {
    if (reset) {
        const ELS = JSON.parse(localStorage.getItem('ELS'))
        Array.from(els).map((vm, index) => {
            vm.style.transition = 'all 600ms ease 0s';
            vm.style.zIndex = '1'
    ...
    if (!localStorage.getItem('ELS')) {
        ...
        localStorage.setItem('ELS',JSON.stringify(temEls))
    } else {
        const ELS = JSON.parse(localStorage.getItem('ELS'))
        Array.from(els).map((vm, index) => {
